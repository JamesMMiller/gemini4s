...
object GeminiErrorCompanion {
   ...
   */
  def invalidApiKey(msg: String = "Invalid API key provided"): GeminiError =
    InvalidApiKey(msg)
  ...
  def unsupportedModel(modelId: String): GeminiError =
    UnsupportedModel(s"Model $modelId is not supported")
  ...
  def contentGenerationFailed(msg: String): GeminiError =
    ContentGenerationFailed(msg)

  def connectionError(msg: String, cause: Throwable): GeminiError =
    ConnectionError(msg, Some(cause))

  def timeoutError: GeminiError = TimeoutError()

  def streamError(msg: String, cause: Option[Throwable] = None): GeminiError =
    StreamInterrupted(msg, cause)
