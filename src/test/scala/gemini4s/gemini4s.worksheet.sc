...
def chatbot(service: Gemini[IO])(using GeminiConfig): IO[Unit] = {
  def chat(history: Ref[IO, List[Content]]): IO[Unit] = for {
    _     <- IO.print("You: ")
    input <- IO.readLine
    _     <- if (input.toLowerCase == "quit") IO.unit
             else {
               ...
               for {
                 ...
                 response <- service
                               .generateContentStream(contents = currentHistory)
